...
def warm_up(model, optim, epoch, loss1, loss2, dataloader, device, crit, aug_times, bs):
    ...
    for epo in range(epoch):
        if epo == epoch - 1:
            model.train()
            datas = np.zeros(shape=(len(dataloader.dataset) * aug_times, 3, 64, 64))
            ...
            for idx, (x, y, f_t, s_idx) in enumerate(dataloader):
                x = torch.cat(x, dim=0)
                y = torch.cat(y, dim=0)
            ...
            print(np.mean(pseudo_labelss == fixed_labelss))
...
def test(model1, model2, loader, device, epoch, bs):
    membership1 = np.zeros(shape=(13000, 10))
    membership2 = np.zeros(shape=(13000, 10))
    ...
    acc, per_class_num, nmi, ari = tools_stl10.clustering_metrix(true_label=raw_label, pre_label=pre_label)
...
def ensemble_selector(prob, aug_times, bs, pseudo_labels, fixed_labels, idx_s, threshold, epoch, number, select_idx):
    ...
    tools_stl10.select_samples_detailed_info(class_num=10, select_idx=np.where(prob_ > threshold)[0], pseudo_labels=labeled_pseudo_label, ground_truth_fixed=fi_t)
...
def labeled_and_unlabeled_loader(select_info, param):
    ...
    labeled_dataset = sep_dataset_stl10.stl10_for_mixup(root='./data/', transform=train_trans,
                                                        pseudo_labels=labeled_pseudo_label, select_index=labeled_index,
                                                        aug_times=param.aug_times)

    unlabeled_dataset = sep_dataset_stl10.stl10_for_mixup(root='./data/', transform=train_trans,
                                                          pseudo_labels=unlabeled_pseudo_label, select_index=unlabeled_index,
                                                          aug_times=param.aug_times)

    labeled_loader = DataLoader(labeled_dataset, batch_size=50, shuffle=False, num_workers=16, drop_last=False)
    unlabeled_loader = DataLoader(unlabeled_dataset, batch_size=50, shuffle=False, num_workers=16, drop_last=False)
...
def mixmatch(model1, model2, w_b, labeled_loader, unlabeled_loader, device, param, semi_loss, sigma, optim):
    ...
    with torch.no_grad():
        ...
        for i in range(param.aug_times):
            labeled_data.append(np.zeros(shape=(len(labeled_loader.dataset), 3, 64, 64)))

        for idx, (x, y) in enumerate(labeled_loader):
            for j in range(param.aug_times):
                x_j = x[j]
                x_j_np = x_j.detach().cpu().numpy()
                labeled_data[j][idx * 50: idx * 50 + x_j_np.shape[0]] = x_j_np
            ...
            pb[idx * 50: idx * 50 + pb_bs.shape[0]] = pb_bs
            labeled_pseudo_y_np[idx * 50: idx * 50 + pb_bs.shape[0]] = y_bs
        ...
        unlabeled_data = []
        for i in range(param.aug_times):
            unlabeled_data.append(np.zeros(shape=(len(unlabeled_loader.dataset), 3, 64, 64)))

        for idx, (x, y) in enumerate(unlabeled_loader):
            for j in range(param.aug_times):
                x_j = x[j]
                x_j_np = x_j.detach().cpu().numpy()
                unlabeled_data[j][idx * 50: idx * 50 + x_j_np.shape[0]] = x_j_np
            ...
            qu[idx * 50: idx * 50 + qu_bs.shape[0]] = qu_bs
        ...
        sub_train_loader0 = DataLoader(dataset=sub_train_set0, batch_size=256, shuffle=True, drop_last=True)
        sub_train_loader1 = DataLoader(dataset=sub_train_set1, batch_size=256, shuffle=True, drop_last=True)
    ...
    pre_matrix = np.zeros(shape=(13000, 10))
    raw_label = np.zeros(shape=(13000, ), dtype=np.int32)
    ...
    for idx, (x, y) in enumerate(test_loader):
        ...
        y_np = y.detach().cpu().numpy()
        raw_label[50 * idx: 50 * (idx + 1)] = y_np
        ...
        pre_matrix[50 * idx: 50 * (idx + 1)] = pred_np1
...
def get_train_trans():
    train_trans = transforms.Compose([transforms.RandomResizedCrop(size=64),
                                      transforms.RandomHorizontalFlip(),
                                      transforms.RandomApply(
                                          [transforms.ColorJitter(0.8 * 0.5, 0.8 * 0.5, 0.8 * 0.5, 0.2 * 0.5)], p=0.8),
                                      transforms.RandomGrayscale(p=0.4),
                                      transforms.RandomApply([GaussianBlur(kernel_size=23)], p=0.6),
                                      transforms.ToTensor()])
    return train_trans


def get_test_trans():
    test_trans = transforms.Compose([transforms.Resize(size=(64, 64)),
                                     transforms.ToTensor()])
    return test_trans