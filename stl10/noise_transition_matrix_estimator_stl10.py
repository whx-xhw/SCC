...
def ntm_estimator(membership, eta, device, model, param):
    ntm_select_idx, ntm_select_pseudo_label = membership_selector_stl10.membership_selector(membership=membership, eta=int(eta * 1300 * 0.02))

    train_trans = get_train_trans()
    estimate_dataset = sep_dataset_stl10.stl10_for_ntm(root='./data/', transform=train_trans, pseudo_labels=ntm_select_pseudo_label, select_index=ntm_select_idx, aug_times=param.aug_times)
    estimate_loader = DataLoader(dataset=estimate_dataset, batch_size=50, shuffle=False, drop_last=False, num_workers=16)
    ...
    for idx, (x, y) in enumerate(estimate_loader):
        ...
        pred[idx * 50 * param.aug_times: idx * 50 * param.aug_times + out_np.shape[0]] = out_np
