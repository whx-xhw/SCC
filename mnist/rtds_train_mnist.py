...
def test(model1, model2, raw_data, ground_truth, device, epoch):
    ...
    raw_data = raw_data.view(-1, 784)
    raw_data = raw_data.float()

    out1 = model1(raw_data)
    out2 = model2(raw_data)

    pre_membership = out1 + out2
    pre_membership = pre_membership.detach().cpu().numpy()
    pre_label = np.argmax(pre_membership, axis=1)
    acc, per_class_num, nmi, ari = tools_mnist.clustering_metrix(true_label=ground_truth, pre_label=pre_label)
...
def labeled_and_unlabeled_loader(absolute_labeled_information, absolute_unlabeled_information):
    ...
    labeled_set = sep_datasets_mnist.MNIST(root='./data/', train=True, transform=trans, noisy_idx=absolute_labeled_index,
                                           noisy_label=labeled_pseudo_label)

    unlabeled_set = sep_datasets_mnist.MNIST(root='./data/', train=True, transform=trans, noisy_idx=absolute_unlabeled_index,
                                             noisy_label=unlabeled_pseudo_label)
...
def input_target_align(labeled_aug_data, unlabeled_aug_data, target_labeled, target_unlabeled, aug_times):
    ...
    aug_data = np.reshape(aug_data, newshape=(-1, 28, 28))
...
def aug_align(aug_data, idx, aug_times):
    ...
    aug_data_select = np.reshape(aug_data_select, newshape=(-1, 28, 28))
...
def mixmatch(model1, model2, optim, labeled_loader, unlabeled_loader, batch_size, device, temp, semi_loss, w_b, aug_data,
             tr_sel_idx, tr_unsel_idx, ground_fixed, aug_times, sigma, model_number, rtds_current_epoch, epoch, data_path):
    model1.train()
    model2.eval()
    ...
    for idx, (labeled_x, labeled_pseudo_y, labeled_idx) in enumerate(labeled_loader):
        ...
        labeled_aug_data_tensor = labeled_aug_data_tensor.view(-1, 784)
    ...
    for idx, (unlabeled_x, unlabeled_pseudo_y, unlabeled_idx) in enumerate(unlabeled_loader):
        ...
        unlabeled_aug_data_tensor = unlabeled_aug_data_tensor.view(-1, 784)
        unlabeled_aug_data_tensor = unlabeled_aug_data_tensor.float()
        x_np = unlabeled_aug_data_tensor.detach().cpu().numpy()
    ...
    #qu_sharpen = torch.tensor(qu_sharpen)
    ...
    aug_da, aug_tar = input_target_align(labeled_aug_data=labeled_aug_data, unlabeled_aug_data=unlabeled_aug_data,
                                         target_labeled=target_labeled, target_unlabeled=target_unlabeled, aug_times=aug_times)
    ...
    sub_train_loader0 = DataLoader(dataset=sub_train_set0, batch_size=1024, shuffle=True, drop_last=False)
    sub_train_loader1 = DataLoader(dataset=sub_train_set1, batch_size=1024, shuffle=True, drop_last=False)
    trans = torchvision.transforms.Compose([torchvision.transforms.ToTensor()])
    test_set = torchvision.datasets.MNIST(root=data_path, download=True, transform=trans, train=True)
    test_loader = DataLoader(dataset=test_set, batch_size=10000, shuffle=False, drop_last=False)
