...
import torch
import numpy as np
from clustering_module_usps import clustering_model
from torchvision import datasets, transforms
from torch.utils.data import DataLoader, TensorDataset
import tools_usps
import os
import membership_selector_usps
from robust_target_distribution_solver_usps import rtds_usps
...
def main():
    ...
    raw_set = datasets.USPS(root='./data', transform=trans, train=True, download=True)
    raw_loader = DataLoader(dataset=raw_set, shuffle=False, batch_size=len(raw_set), drop_last=False)

    raw_data = torch.zeros(size=(len(raw_set), 1, 16, 16))
    ...
    for idx, (x, y) in enumerate(raw_loader):
        x = x.to(device)
        y = y.to(device)
        raw_data[len(raw_set) * idx: len(raw_set) * (idx + 1)] = x
        raw_label[len(raw_set) * idx: len(raw_set) * (idx + 1)] = y
        x_flatten = x.view(-1, 256)
        init_membership[len(raw_set) * idx: len(raw_set) * (idx + 1)] = clustering_module(x_flatten)
    ...
    accuracy, per_class_correct, nmi, ari = tools_usps.clustering_metrix(true_label=raw_label_np, pre_label=init_pseudo_label)
    ...
    if os.path.exists(pre_aug_data_path):
        ...
        tools_usps.auto_augmentation(aug_times=param.aug_times, ground_truth=raw_label_np, ground_truth_fixed=ground_truth_fixed)
        pre_aug_data = np.load(pre_aug_data_path)
    ...
    while True:
        ...
        select_idx, select_pseudo_label = membership_selector_usps.membership_selector(membership=membership, eta=eta)
        selected_ground_truth = raw_label_np[select_idx]
        acc, per_class_num = tools_usps.cluster_acc(y_true=selected_ground_truth, y_pred=select_pseudo_label)
        print('Epoch {}, selected samples, acc:{}'.format(epoch, acc))

        target_distribution = rtds_usps(param=param, select_idx=select_idx, select_pseudo_label=select_pseudo_label, device=device,
        ...
        acc, per_class_num = tools_usps.cluster_acc(y_true=raw_label_np, y_pred=target_distribution_pseudo_label)
        ...
        clustering_module_optim_loader = DataLoader(dataset=clustering_module_optim_set, batch_size=128, shuffle=True, drop_last=False, num_workers=16)
        ...
        for mm in range(20):
            ...
            raw_data_tensor_cuda = raw_data_tensor_cuda.view(-1, 256)
            ...
            acc, per_class_num, nmi, ari = tools_usps.clustering_metrix(true_label=raw_label_np, pre_label=pseudo_label)
        ...
        last_current_distribution_sim, _, _, _ = tools_usps.clustering_metrix(true_label=current_distribution, pre_label=last_distribution)
