...
def auto_augmentation(aug_times, ground_truth, ground_truth_fixed):
    pre_set = datasets.USPS(root='./data/', transform=transforms.ToTensor(), download=True, train=True)
    pre_loader = DataLoader(dataset=pre_set, shuffle=False, batch_size=len(pre_set), drop_last=False)

    pre_aug_data = np.zeros(shape=(len(pre_set), aug_times, 16, 16))
    pre_aug_label = np.zeros(shape=(len(pre_set), aug_times), dtype=np.int32)
    ...
    for idx, (x, y) in enumerate(pre_loader):
        ...
        for i in range(len(pre_set)):
            ...
            print('automate augmentation: {}/{}'.format(counter + 1, len(pre_set)))

            p = Augmentor.Pipeline(dic_path)
            p.rotate(probability=0.8, max_left_rotation=20, max_right_rotation=15)
            p.random_distortion(probability=0.8, grid_width=3, grid_height=3, magnitude=2)
            p.skew(probability=0.8, magnitude=0.7)
    ...
    pre_aug_data_aligned = np.zeros(shape=(len(pre_set) * aug_times, 16, 16), dtype=np.float32)
    pre_aug_label_aligned = np.zeros(shape=(len(pre_set) * aug_times,), dtype=np.int64)
    ...
    np.save('./data/pre_aug_data_aligned.npy', pre_aug_data_aligned)
    np.save('./data/pre_aug_label_aligned.npy', pre_aug_label_aligned)