...
from noise_transition_matrix_estimator_usps import *


class classifier1(nn.Module):
    def __init__(self):
        super(classifier1, self).__init__()
        self.Linear1 = nn.Linear(256, 1000)
...
class classifier2(nn.Module):
    def __init__(self):
        ...
        self.Linear1 = nn.Linear(256, 300)
...
def rtds_usps(param, select_idx, select_pseudo_label, device, epoch, aug_data, ground_truth_fixed, raw_data_tensor, membership, eta):
    ...
    dual_model_train_loader = DataLoader(dataset=dual_model_train_set, batch_size=1024, shuffle=True, drop_last=False, num_workers=16)
    ...
    optim1 = torch.optim.SGD(params=dual_model1.parameters(), lr=0.01, momentum=0.99)
    optim2 = torch.optim.SGD(params=dual_model2.parameters(), lr=0.01, momentum=0.99)

    print('Epoch: {}, warming up for sample selection...'.format(epoch))
    dual_model1 = warm_up(model=dual_model1, optim=optim1, epoch=10, loss1=CE_Loss, loss2=Conf_Penalty,
                          dataloader=dual_model_train_loader, device=device, crit=1.0)

    dual_model2 = warm_up(model=dual_model2, optim=optim2, epoch=10, loss1=CE_Loss,loss2=Conf_Penalty,
                          dataloader=dual_model_train_loader, device=device, crit=1.0)
    ...
    labeled_idx1, unlabeled_idx1, w_b1 = ensemble_selector(prob=prob1, aug_times=param.aug_times, upsilon=0.5,
                                                           pseudo_label=select_pseudo_label, ground_truth_fixed=select_ground_truth_fixed,
    ...
    prob2, loss2 = eval(model=dual_model2, data_loader=dual_model_eval_loader, device=device, loss=CE,
                        eval_number=select_augmented_data.shape[0])
    ...
    for c_epo in range(50):
        dual_model2 = mixmatch(model1=dual_model2, model2=dual_model1, optim=optim2, labeled_loader=labeled_loader1,
                               ...
                               tr_unsel_idx=absolute_unlabeled_index1, aug_times=param.aug_times, sigma=sigma)
        ...
        dual_model1 = mixmatch(model1=dual_model1, model2=dual_model2, optim=optim1, labeled_loader=labeled_loader2,
                               ...
                               tr_unsel_idx=absolute_unlabeled_index2, aug_times=param.aug_times, sigma=sigma)

        raw_data_tensor_flatten = raw_data_tensor.to(device)
        raw_data_tensor_flatten = raw_data_tensor_flatten.view(-1, 256)
        ...
        accuracy, per_class_correct, nmi, ari = tools_usps.clustering_metrix(true_label=ground_truth_fixed,
